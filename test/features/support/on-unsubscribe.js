...
Given(/^SP has previously sent an unsubscribe request to CRVS$/, function () {
  specUnsubscribe = spec(); // Initialize the specSearch object
});

When(/^CRVS completes processing and calls SP on-unsubscribe callback$/, async function () {
  try {
...
Then(/^SP should receive the on-unsubscribe response from CRVS$/, async function () {
